...
function isQuestionEligible(operator, questionMode) {
  const mode = normalizeQuestionMode(questionMode);
  if (mode === "simple") return Number(operator.rarity) >= 3;
  if (mode === "beginner") return Number(operator.rarity) >= 5;
  return true;
}
...
function answerView(operator) {
  return {
    id:operator.id, name:operator.name, en:operator.en, rarity:operator.rarity,
    class:operator.class, subclass:operator.subclass, faction:operator.faction,
    group:operator.group, year:operator.year, tags:[...operator.tags],
    position:[...operator.position], hasAlter:Boolean(operator.hasAlter), alterOf:operator.alterOf || null
  };
}
...
function overlaps(left, right) { return (left || []).some(value => (right || []).includes(value)); }
function numericFeedback(guess, target, closeDistance = 1) {
  if (guess === target) return "exact";
  const relation = Math.abs(guess - target) <= closeDistance ? "close" : "miss";
  return `${relation}-${target > guess ? "up" : "down"}`;
}

export function evaluateArknightsGuess(guess, target) {
  return {
    nickname:guess.id.toLowerCase() === target.id.toLowerCase() ? "exact" : "miss",
    rarity:numericFeedback(guess.rarity, target.rarity),
    class:guess.class === target.class ? "exact" : "miss",
    subclass:guess.subclass === target.subclass ? "exact" : guess.class === target.class ? "close" : "miss",
    faction:guess.faction === target.faction ? "exact" : guess.group === target.group ? "close" : "miss",
    year:numericFeedback(guess.year, target.year),
    tags:sameSet(guess.tags, target.tags) ? "exact" : overlaps(guess.tags, target.tags) ? "close" : "miss",
    position:sameSet(guess.position, target.position) ? "exact" : overlaps(guess.position, target.position) ? "close" : "miss",
    alter:Boolean(guess.hasAlter) === Boolean(target.hasAlter) ? "exact" : "miss"
  };
}

export class ArknightsGameManager {
  constructor(players, options = {}) {
    if (!Array.isArray(players) || !players.length) throw new Error("ArknightsGameManager requires operators");
  ...
  pickTarget(questionMode = "full") {
    const mode = normalizeQuestionMode(questionMode);
    const eligible = this.players.filter(operator => isQuestionEligible(operator, mode));
    const pool = eligible.length ? eligible : this.players;
  ...
  guessSingle(id, accessToken, playerId) {
    ...
    if (!guess) throw Object.assign(new Error("没有找到这位干员"), { status:400 });
    ...
    if (game.guesses.some(row => row.player.id.toLowerCase() === guess.id.toLowerCase())) throw Object.assign(new Error("这位干员已经猜过"), { status:409 });
    const feedback = evaluateArknightsGuess(guess, game.target);
  ...
  guessRoom(code, accessToken, playerId) {
    ...
    if (participant.guesses.some(row => row.player.id.toLowerCase() === guess.id.toLowerCase())) throw Object.assign(new Error("这位干员已经猜过"), { status:409 });
    const feedback = evaluateArknightsGuess(guess, room.target);
...
export const ARKNIGHTS_GAME_CONSTANTS = { MAX_GUESSES, SETTLEMENT_MS, ROUND_INTERMISSION_MS, MATCH_DURATION_MS, MATCHMAKING_SERIES, MIN_ROUND_DURATION_SECONDS, MAX_ROUND_DURATION_SECONDS, REMATCH_WINDOW_MS, ROOM_INACTIVITY_MS, SERIES_OPTIONS, PRIVATE_PLAYER_OPTIONS, MIN_SCORE_TO_WIN, MAX_SCORE_TO_WIN, QUESTION_MODES };
